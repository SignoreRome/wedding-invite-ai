...
- MVP-пасхалка `Dino Bride.exe`: миниигра в стиле Chrome Dino, запускаемая вручную через кнопку `Start`
...
- Игра запускается только явным действием пользователя: кнопкой `Start game`, тапом по canvas или клавишей Space/ArrowUp после открытия окна.
...
- Счет, собранные кольца и лучший результат живут только в состоянии открытой страницы. Сервер, Prisma, RSVP API и схема SQLite для миниигры не используются.
