...
- MVP-пасхалка `Dino Bride.exe`: миниигра в стиле Chrome Dino, запускаемая вручную через кнопку `Start`, с выбором режима, сюжетным финалом и сохранением попыток
...
- При открытии окна сначала нужно выбрать режим: `Сюжетный` или `На рекорд`. В сюжетном режиме строка статистики скрыта, а режим `На рекорд` оставляет счет, кольца и лучший результат на экране.
- Игра запускается только явным действием пользователя после выбора режима: кнопкой `Start game`, тапом по canvas или клавишей Space/ArrowUp после открытия окна.
...
- При столкновении игра показывает сообщение `Потрачено`, сохраняет неуспешную попытку в `POST /api/game/attempts` и оставляет имя игрока пустым.
...
- На телефоне можно играть тапом по полю или кнопкой `Прыжок`; canvas ограничен шириной окна и не должен создавать горизонтальный скролл.
...
- `playerName` — optional строка до 80 символов, сейчас отправляется как `null`
