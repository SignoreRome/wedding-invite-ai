...
- MVP-пасхалка `Dino Bride.exe`: миниигра в стиле Chrome Dino, запускаемая вручную через кнопку `Start`, с выбором режима и сохранением попыток
...
- При открытии окна сначала нужно выбрать режим: `Сюжетный` или `На рекорд`. На текущем этапе режимы используют один и тот же runner-геймплей; отдельная логика сюжета и рекордов остается будущей задачей.
...
- Спрайты динозавра лежат в [`public/dino-bride.png`](./public/dino-bride.png) и [`public/dino-bride-run.png`](./public/dino-bride-run.png), кольцо — в [`public/ring.png`](./public/ring.png), а препятствия выбираются из существующих иконок в [`public/desktop-icons/`](./public/desktop-icons/).
...
- При столкновении игра показывает сообщение `Потрачено`, сохраняет попытку в `POST /api/game/attempts` и оставляет имя игрока пустым.
...
- Необходимая дистанция для будущего прохождения сюжетного режима хранится в `game_settings.story_required_distance`. Дефолтное значение — `2500`.
