...
- Секция опроса гостей на публичной странице отправляет RSVP-ответы в `POST /api/rsvp` и включает поля: ФИО, присутствие, `+1`, имя пары, трансфер после банкета и комментарий гостя.
...
- `POST /api/rsvp` валидирует JSON через Zod, нормализует ФИО гостя, обрезает пробелы у строк, ограничивает длину полей и сохраняет данные через Prisma.
- При повторной отправке с тем же ФИО гостя API обновляет существующую RSVP-запись, а не создает новую строку; сравнение не учитывает лишние пробелы и регистр.
