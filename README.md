...
- Интерактивные XP-кнопки окна: сворачивание с desktop-иконками, разворачивание, закрытие с предупреждением и меню `Файл` / `Правка` / `Вид` / `Справка` с фото
...
- Админские страницы `/admin/rsvp` и `/admin/game`, защищенные общим паролем из env
- MVP-пасхалка `Dino Bride.exe`: миниигра в стиле Chrome Dino, запускаемая вручную через кнопку `Start`, с меню режимов, таблицей лидеров endless, сюжетным финалом и сохранением попыток
- Таблицы `game_attempts` и `game_settings` для статистики и базовых параметров миниигры
...
prisma/
  migrations/
    20260426000000_init/
      migration.sql
    20260429000000_add_rsvp_info/
      migration.sql
    20260430000000_add_dino_game_tables/
      migration.sql
  schema.prisma
prisma.config.ts
data/
...
Для проверки с телефона в той же Wi-Fi/LAN-сети откройте сетевой адрес из вывода `npm run dev`, например `http://192.168.0.217:3000`. `next.config.ts` автоматически добавляет локальные IPv4-адреса машины в `allowedDevOrigins`, чтобы Next.js dev-ресурсы не блокировались при открытии сайта с мобильного устройства. После смены сети или IP перезапустите dev-сервер.
...
- `ADMIN_RSVP_PASSWORD` — общий пароль для доступа к административным страницам `/admin/rsvp` и `/admin/game`; если переменная не задана, данные в админке не показываются
...
Админские страницы `/admin/rsvp` и `/admin/game` хранят вход в общей httpOnly cookie на 12 часов. Если сайт открыт по HTTPS за Nginx, оставьте `ADMIN_RSVP_COOKIE_SECURE` unset и передавайте `X-Forwarded-Proto`, как в примере ниже. Если во время настройки VPS админка открывается напрямую по `http://IP:3000`, задайте `ADMIN_RSVP_COOKIE_SECURE="false"`, иначе браузер будет отбрасывать Secure-cookie и после обновления, сортировки или сохранения настроек снова показывать форму ввода пароля.
...
- Кнопка `Start` в нижней панели приглашения открывает отдельное XP-окно `Dino Bride.exe`.
- При открытии окна показывается меню: кнопки режимов `Сюжет` и `Рекорд`, кнопка `Управление` с подсказками по прыжку и `Доска почета` с тремя лучшими результатами endless mode.
- В сюжетном режиме строка статистики скрыта. В режиме `Рекорд` над игрой показывается `Король Дино`, а текущий счет остается внутри игрового поля.
- Игра запускается только явным действием пользователя: нажатием режима `Сюжет` или `Рекорд` в меню. После запуска под игрой доступны кнопки `Меню` и `Начать заново`.
- Миниигра реализована на Canvas в [`src/components/easter-eggs/dino-minigame.tsx`](./src/components/easter-eggs/dino-minigame.tsx), без новых зависимостей и без игровых движков.
- Спрайты динозавра лежат в [`public/dino-bride.png`](./public/dino-bride.png), [`public/dino-bride-run.png`](./public/dino-bride-run.png) и [`public/dino-dress.png`](./public/dino-dress.png), сердце финала — в [`public/pixel-heart.png`](./public/pixel-heart.png), кольцо — в [`public/ring.png`](./public/ring.png), а препятствия выбираются из существующих иконок в [`public/desktop-icons/`](./public/desktop-icons/).
- Счет складывается из дистанции и бонусов за кольца: `floor(distance / 32) + ringBonusScore`. Первое кольцо дает 120 очков, каждое следующее — на 15 очков больше предыдущего.
- Collision box препятствий намеренно меньше видимого desktop-icon спрайта, чтобы проигрыш не срабатывал от касания прозрачных или декоративных краев.
- При столкновении игра показывает крупное сообщение `ПОТРАЧЕНО` без дополнительной статистики. Если счет в endless mode попадает в топ-3, игроку предлагается ввести имя до 30 символов; пустое имя отображается как `Гость`.
- Множитель скорости для режима `На рекорд` читается из таблицы `game_settings`, колонка `endless_speed_multiplier`. Дефолтное значение — `1`.
- Необходимая дистанция для финала сюжетного режима хранится в `game_settings.story_required_distance`. Дефолтное значение — `2500`; после достижения дистанции игра показывает динозаврика в платье, сердце и надпись `Встретимся на свадьбе 07/08/26`, а попытка сохраняется как успешная.
- На телефоне прыжок выполняется тапом по игровому полю; canvas ограничен шириной окна и не должен создавать горизонтальный скролл.
...
- Миграция [`prisma/migrations/20260430000000_add_dino_game_tables/migration.sql`](./prisma/migrations/20260430000000_add_dino_game_tables/migration.sql) добавляет таблицы миниигры `game_attempts` и `game_settings`.
- RSVP-модель `Rsvp` мапится на SQLite-таблицу `rsvp`.
...
- Таблица `game_attempts` содержит колонки: `id`, `mode`, `player_name`, `score`, `coins_collected`, `distance`, `is_success`, `created_at`.
- Таблица `game_settings` содержит singleton-строку с `id = 1` и колонками `story_required_distance`, `endless_speed_multiplier`, `updated_at`.
- При применении миграций Prisma дополнительно создает служебную таблицу `_prisma_migrations`.
...
`GET /api/game/settings` возвращает настройки миниигры из singleton-строки `game_settings.id = 1`:
...
`GET /api/game/attempts` возвращает публичную таблицу лидеров endless mode: три лучшие попытки по счету, с ранним временем и меньшим `id` как tie-breaker:
...
`POST /api/game/attempts` сохраняет попытку после окончания игры:
...
- `mode` — строка `story` или `endless`
- `playerName` — optional строка до 30 символов; public UI запрашивает ее только для endless-результата, который попадает в топ-3
- `score`, `coinsCollected`, `distance` — неотрицательные целые числа
- `isSuccess` — boolean; сервер сохраняет `true` только для `story`, если дистанция не меньше `story_required_distance`

Для ручной настройки параметров откройте `/admin/game`: страница редактирует singleton-строку `game_settings.id = 1` и показывает текущие значения `story_required_distance` и `endless_speed_multiplier`. `story_required_distance` должен быть целым числом от `1` до `10000000`, `endless_speed_multiplier` должен быть от `0.01` до `3`.

### Административное управление игрой

- Страница доступна по маршруту `/admin/game`.
- Доступ защищен тем же паролем `ADMIN_RSVP_PASSWORD` и той же cookie-сессией для `/admin`, что и `/admin/rsvp`.
- Блок управления настройками редактирует таблицу `game_settings`: `story_required_distance` для финала сюжетного режима и `endless_speed_multiplier` для скорости режима `На рекорд`.
- Настройки валидируются на сервере перед сохранением.
- Информационный блок показывает статистику из `game_attempts`: всего сыгранных игр по обоим режимам, количество успешных прохождений story mode, максимальный счет endless mode, имя игрока с максимальным счетом и последние 10 записей.
